...
function useCardPrefs(cardId: string, defaultColor: string) {
  ...
  const save = (next: any) => { try { localStorage.setItem(storageKey, JSON.stringify(next)) } catch {} }
...
function AthleteStatCard({ card, entries }: { card: any; entries: any[] }) {
  ...
  const chartData = entries.map((e: any) => {
    ...
    return { date: new Date(e.recordedAt).toLocaleDateString('de-DE', { day: '2-digit', month: 'short' }), value: displayValue, _real: displayValue }
  })
  ...
  const trend = latestDisplay != null && firstDisplay != null ? +(latestDisplay - firstDisplay).toFixed(2) : null

  const trendPositiveIsGood = !isPaceCard && !(isWeightCard && weightGoal === 'lose')
  ...
  const latestDisplayValue = (() => {
    if (!latest) return '—'
    if (isPaceCard && latest.secondaryValue && latest.value) return (latest.secondaryValue / latest.value).toFixed(2)
    if (isSpeedCard && latest.secondaryValue && latest.value) return (latest.value / (latest.secondaryValue / 60)).toFixed(1)
    return latest.value
  })()
    ...
    <div className="bg-white/3 border rounded-2xl p-5 relative" style={{ borderColor: `${cardColor}25` }}>
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="text-right">
          <p className="font-semibold" style={{ color: cardColor }}>{latestDisplayValue}</p>
          {trend !== null && trend !== 0 && (
      ...
      try {
        ...
        if (saved) {
          ...
          const sorted = [...athleteStats].sort((a, b) =>
            savedIds.indexOf(a.card._id) - savedIds.indexOf(b.card._id)
          )
          setOrderedStats(sorted)
        } else {
          setOrderedStats(athleteStats)
        }
      } catch { setOrderedStats(athleteStats) }
    } else {
      setOrderedStats([])
    }
        ...
        {isLoading ? (
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            {relations.map((rel: any) => (
              <button key={rel._id}
                onClick={() => setSelectedAthleteId((prev) => prev === rel.athleteId._id ? null : rel.athleteId._id)}
        ...
        {selectedAthleteId && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-medium">{selectedRelation?.athleteId?.name}'s progress</h2>
            ...
            {orderedStats.length === 0 ? (
              ...
              <DndContext
                ...
                onDragEnd={(event) => {
                  ...
                  setOrderedStats((prev) => {
                    const oldIndex = prev.findIndex((s) => s.card._id === active.id)
                    const newIndex = prev.findIndex((s) => s.card._id === over.id)
                ...
                <SortableContext items={orderedStats.map((s) => s.card._id)} strategy={rectSortingStrategy}>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {orderedStats.map(({ card, entries }: any) => (
                      <SortableStatCard key={card._id} card={card} entries={entries} />
                    ))}
                  </div>
                </SortableContext>
              </DndContext>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {orderedStats.map(({ card, entries }: any) => (
                  <AthleteStatCard key={card._id} card={card} entries={entries} />
                ))}
              </div>
            )}
          </div>
