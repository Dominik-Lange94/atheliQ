...
function SortableStatCard({ card, latest, selected, onToggleSelect }: {
  card: any; latest: any; selected: boolean; onToggleSelect: () => void;
}) {
  ...
  return (
    <div ref={setNodeRef}
      ...
      <div className="pointer-events-none opacity-60">
        <StatCard card={card} latest={latest} selected={selected} onToggleSelect={onToggleSelect} />
      </div>
    </div>
  );
}
...
export default function AthleteDashboard() {
  ...
  const handleDragEnd   = (event: DragEndEvent) => {
    ...
    setOrderedCards((prev) => {
      ...
      api.patch("/athlete/cards/reorder", {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 bg-white/3 border border-white/10 rounded-xl px-3 py-2 flex-shrink-0">
            ...
            <button
              onClick={() => { setSelectedDate(TODAY); setWindowOffset(0); }}
              disabled={isToday}
        ...
        <div>
          <div className="flex items-center justify-between mb-4 gap-2">
            <div className="flex-shrink-0">
              ...
              <p className="text-slate-400 text-xs mt-0.5">
                {dayLoading ? "Lade…" : isToday ? "Aktuellste Werte" : formatDateDisplay(selectedDate)}
              </p>
            </div>
          ...
          {cardsLoading ? (
            ...
            <div className="border border-dashed border-white/10 rounded-2xl p-12 text-center">
              <p className="text-slate-400 text-sm">No cards yet — add your first metric above</p>
            </div>
          ) : arrangeMode ? (
                ...
                return (
                  ...
                  >
                    <span className={`text-[9px] font-medium uppercase tracking-wide ${
                      isSelected ? "text-[#FFD300]" : isT ? "text-slate-300" : "text-slate-500"
                    }`}>
                      {weekday}
                    </span>
                    <span className={`text-xs font-semibold leading-none ${
                      isSelected ? "text-[#FFD300]" : isT ? "text-white" : "text-slate-400"
                    }`}>
                      {day}
                    </span>
                    {isT && !isSelected && <span className="w-1 h-1 rounded-full bg-[#FFD300]/50 mt-0.5" />}
                    {isSelected         && <span className="w-1 h-1 rounded-full bg-[#FFD300] mt-0.5" />}
                  </button>
                );
              })}
            </div>
