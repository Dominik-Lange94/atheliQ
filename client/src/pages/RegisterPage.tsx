    ...
    <div className="min-h-screen bg-[#0f0f13] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-400 text-sm mt-1">Start tracking your performance</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 p-1 bg-white/5 rounded-xl border border-white/10">
            {(['athlete', 'coach'] as const).map((r) => (
              <label key={r} className={`flex items-center justify-center gap-2 py-2.5 rounded-lg cursor-pointer transition-all text-sm font-medium ${role === r ? 'bg-[#FFD300] text-[#0f0f13]' : 'text-slate-400 hover:text-slate-200'}`}>
        ...
        <p className="text-center text-slate-400 text-sm mt-6">
          ...
          <Link to="/login" className="text-[#FFD300] hover:text-[#FFD300] transition-colors">
            Sign in
          </Link>
        </p>
      </div>
