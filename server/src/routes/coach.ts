...
async function pushSystemMessage(params: {
  ...
  text: string;
  meta: {
    type: "user" | "connect_request" | "connect_accepted" | "connect_declined" | "permission_update";
...
router.get("/athletes/activity", requireRole("coach"), async (req: AuthRequest, res: Response) => {
  try {
    const { from, to } = req.query;
    if (!from && !to) {
    ...
    const result = await Promise.all(
      relations.map(async (rel: any) => {
        ...
        return { athleteId: rel.athleteId._id.toString(), isActive: !!anyEntry };
      })
    );
...
router.get("/search", requireRole("athlete"), async (req: AuthRequest, res: Response) => {
  ...
  try {
    const coach = await User.findOne({ email, role: "coach" }).select("_id name email role");
    if (!coach) { res.status(404).json({ success: false, error: "Kein Coach mit dieser Email gefunden" }); return; }
    if (coach._id.toString() === req.user!.userId) { res.status(400).json({ success: false, error: "Du kannst dich nicht selbst als Coach hinzufügen" }); return; }
...
router.post("/connect/:coachId", requireRole("athlete"), async (req: AuthRequest, res: Response) => {
  try {
    const athleteId = req.user!.userId;
    const coachId   = req.params.coachId;
    if (!mongoose.Types.ObjectId.isValid(coachId)) { res.status(400).json({ success: false, error: "Invalid coach id" }); return; }
    if (athleteId === coachId) { res.status(400).json({ success: false, error: "Du kannst dich nicht selbst verbinden" }); return; }

    const coach = await User.findOne({ _id: coachId, role: "coach" }).select("_id name email");
    if (!coach) { res.status(404).json({ success: false, error: "Coach nicht gefunden" }); return; }

    let relation = await CoachAthlete.findOne({ coachId, athleteId });
    if (relation) {
      if (relation.status === "active")  { res.status(400).json({ success: false, error: "Coach bereits verbunden" }); return; }
      if (relation.status === "pending") { res.status(400).json({ success: false, error: "Anfrage bereits gesendet" }); return; }
      relation.status = "pending";
      relation.allowedMetrics = [];
      await relation.save();
    } else {
      relation = await CoachAthlete.create({ coachId, athleteId, status: "pending", allowedMetrics: [] });
    }

    const thread = await ensureThreadForRelation(relation);
    await pushSystemMessage({
      threadId: thread._id, senderId: athleteId, receiverId: coachId,
...
router.patch("/permissions/:coachId", requireRole("athlete"), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const relation = await CoachAthlete.findOne({ coachId, athleteId, status: "active" });
    if (!relation) { res.status(404).json({ success: false, error: "Active relationship not found" }); return; }

    const previousMetricIds = (relation.allowedMetrics ?? []).map((id: any) => id.toString());
    const nextMetricIds     = parsed.data.allowedMetrics.map((id) => id.toString());
    relation.allowedMetrics = parsed.data.allowedMetrics as any;
    await relation.save();

    const addedMetricIds   = nextMetricIds.filter((id) => !previousMetricIds.includes(id));
    const removedMetricIds = previousMetricIds.filter((id) => !nextMetricIds.includes(id));

    if (addedMetricIds.length || removedMetricIds.length) {
      const changedCards = await StatCard.find({ _id: { $in: [...addedMetricIds, ...removedMetricIds] }, athleteId }).select("_id label");
      const labelMap     = new Map(changedCards.map((card: any) => [card._id.toString(), card.label]));
      const addedLabels  = addedMetricIds.map((id) => labelMap.get(id)).filter(Boolean) as string[];
      const removedLabels = removedMetricIds.map((id) => labelMap.get(id)).filter(Boolean) as string[];
      const messageParts: string[] = [];
      if (addedLabels.length)   messageParts.push(`Freigegeben: ${addedLabels.join(", ")}`);
      if (removedLabels.length) messageParts.push(`Entfernt: ${removedLabels.join(", ")}`);
      const thread = await ensureThreadForRelation(relation);
      await pushSystemMessage({
        threadId: thread._id, senderId: athleteId, receiverId: coachId,
        text: messageParts.join(" • ") || "Metrik-Berechtigungen wurden geändert",
        meta: { type: "permission_update", metricIds: [...addedMetricIds, ...removedMetricIds].map((id) => new mongoose.Types.ObjectId(id)) },
      });
    }
...
router.delete("/disconnect/:coachId", requireRole("athlete"), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!relation) { res.status(404).json({ success: false, error: "Relationship not found" }); return; }
...
router.get("/my-coaches", requireRole("athlete"), async (req: AuthRequest, res: Response) => {
  try {
    ...
    }).populate("coachId", "name email").sort({ updatedAt: -1 });
    res.json({ success: true, data: relations });
  } catch (error) {
